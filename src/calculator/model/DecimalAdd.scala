...
class DecimalAdd(calculator: Calculator) extends State(calculator) {
  ...
  override def equalsPressed(): Unit = {
    /*calculator.LHS = calculator.RHS + calculator.LHS
    calculator.RHS = calculator.LHS
    calculator.displayed = calculator.LHS.toString
    calculator.state = new Add(calculator)
    calculator.RHS = 0
    */
